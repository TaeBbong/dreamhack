...
XSS 문제들과 동일하게 selenium으로 관리자가 로컬호스트에서 접속하는 시나리오를 구성해두었다.
결국 /flag 페이지는 악성 스크립트가 담긴 게시글을 작성하는 행위를 구현해놓은 것이고,
/vuln 페이지는 악성 스크립트 삽입이 가능한 게시판이다.
여기서 악성 스크립트를 넣으면 관리자가 로컬호스트에서 접속을 하는 것이다.

해당 문제의 목적은 관리자가 로컬호스트에서 /admin/notice_flag에 접속하게끔 하는 것이다.
그러면 서버는 신뢰하는 사용자가 접속한 것이므로, 접속이 된다.

`<script>` 태그는 막혀있으므로, `<img>` 태그를 활용한다.
